...
-- SMODS MOD SYSTEM INTEGRATION
-- Load required modules
local nativefs = require "SMODS.libs.nativefs.init"
local json_module = require "SMODS.libs.json.init"
...
-- Load SMODS modules
local function load_sm_mod(path)
    local full_path = SMODS.path .. path
    ...
    if not content then
        error("Could not read SMODS file: " .. full_path)
    end
    local chunk = assert(load(content, ('=[SMODS _ "%s"]'):format(path)))
    return chunk()
end

-- Load SMODS modules with error handling
local smods_modules = {
    "src/ui.lua",
    "src/index.lua", 
    "src/utils.lua",
...
for _, module_path in ipairs(smods_modules) do
    local success, err = pcall(load_sm_mod, module_path)
    if not success then
        print("Error loading SMODS module " .. module_path .. ": " .. tostring(err))
    end
end
